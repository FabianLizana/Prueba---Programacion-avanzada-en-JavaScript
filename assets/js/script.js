...
const cargarImagen = async (srcImage) => {
...
const mostrarAnimalAgregado = (objetoAnimal) => {
...
const reproducirSonido = (elementoAgregado) => {
  ...
  botonSonido.on("click", () => {
    const audioTag = $("#player");
    audioTag.html(`<source src=${botonSonido.attr(
      "data-sonido"
    )} type="audio/mpeg">  
...
const resetearFormulario = (selectAnimal, selectEdad, textAreaComentarios) => {
  selectAnimal.val("");
  selectEdad.val("");
  textAreaComentarios.val("");
  const img = $("#preview");
...
//-------modal--------//
const cargarModal = (elementoAgregado, objeto) => {
